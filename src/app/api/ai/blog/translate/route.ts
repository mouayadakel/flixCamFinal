...
import { translateSchema } from '@/lib/validators/blog-ai.validator'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { content, from, to } = translateSchema.parse(body)
    const translated = await BlogAIService.translate(content, from, to)
    const durationMs = Date.now() - start
    const inputTokens = estimateInputTokens(body)
    const outputTokens = estimateOutputTokens(translated)
