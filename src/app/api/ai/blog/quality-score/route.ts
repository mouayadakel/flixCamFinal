...
import { qualityScoreSchema } from '@/lib/validators/blog-ai.validator'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { content } = qualityScoreSchema.parse(body)
    const result = await BlogAIService.qualityScore(asContentString(content))
