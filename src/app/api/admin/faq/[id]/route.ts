/**
 * GET /api/admin/faq/[id] - Get FAQ item by ID
 * PATCH /api/admin/faq/[id] - Update FAQ item
 * DELETE /api/admin/faq/[id] - Soft delete FAQ item
 */
...
import { updateFaqSchema } from '@/lib/validators/faq.validator'
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await FaqService.update(id, data, session.user.id)
...
export async function DELETE(
  ...
  try {
    const { id } = await params
    await FaqService.delete(id, session.user.id)
