/**
 * GET /api/admin/hero-banners/[id] - Get banner by ID with slides
 * PATCH /api/admin/hero-banners/[id] - Update banner
...
import { updateBannerSchema } from '@/lib/validators/hero-banner.validator'
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await HeroBannerService.updateBanner(id, data, session.user.id)
...
export async function DELETE(
  ...
  try {
    const { id } = await params
    await HeroBannerService.deleteBanner(id, session.user.id)
