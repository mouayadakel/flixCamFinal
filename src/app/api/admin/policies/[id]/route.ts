/**
 * GET /api/admin/policies/[id] - Get policy item by ID
 * PATCH /api/admin/policies/[id] - Update policy item
 * DELETE /api/admin/policies/[id] - Soft delete policy item
 */
...
import { updatePolicySchema } from '@/lib/validators/policy.validator'
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await PolicyService.update(id, data, session.user.id)
...
export async function DELETE(
  ...
  try {
    const { id } = await params
    await PolicyService.delete(id, session.user.id)
