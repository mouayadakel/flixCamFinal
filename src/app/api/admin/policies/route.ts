/**
 * GET /api/admin/policies - List all policy items
 * POST /api/admin/policies - Create policy item
 */
...
import { createPolicySchema } from '@/lib/validators/policy.validator'
...
export async function GET(_request: NextRequest) {
  ...
  try {
    const list = await PolicyService.getAll()
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const created = await PolicyService.create(data, session.user.id)
