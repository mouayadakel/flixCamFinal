/**
 * GET /api/admin/shoot-types/[id] - Get shoot type by ID
 * PATCH /api/admin/shoot-types/[id] - Update shoot type
 * DELETE /api/admin/shoot-types/[id] - Soft delete shoot type
 */
...
import { updateShootTypeSchema } from '@/lib/validators/shoot-type.validator'
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const shootType = await ShootTypeService.getById(id)
    return NextResponse.json(shootType)
  } catch (error) {
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = updateShootTypeSchema.parse({ ...body, id })
    const updated = await ShootTypeService.update(id, data, session.user.id)
...
export async function DELETE(
  ...
  try {
    const { id } = await params
    await ShootTypeService.softDelete(id, session.user.id)
