    ...
    <div className="space-y-4">
            ...
            {Array.from({ length: daysInMonth }, (_, i) => {
              ...
              return (
                <button
                  ...
                  className={`
                    relative flex h-10 items-center justify-center rounded-lg border text-sm font-medium transition-all
                    ${STATUS_COLORS[status] ?? STATUS_COLORS.past}
      ...
      {todaySched && !todaySched.isClosed && (
        <p className="text-xs text-muted-foreground">
          ...
          <span className="font-medium">{todaySched.openTime} – {todaySched.closeTime}</span>
        </p>
      )}
    </div>
  )
